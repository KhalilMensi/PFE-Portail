                ...
                },
                {
                    ...
                    "render": function (data) {
                        ...
                        <div class="col-md-3 text-center">
                             <a href="/Ebook/DetailsEbook?id=${data}" target="_blank">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#0e49b5" class="bi bi-book" viewBox="0 0 16 16">
                         ...
                         <div class="col-md-3 text-center">
                             <a href="/Ebook/UpsertEbook?id=${data}" target="_blank">  
                        ...
                        <div class="col-md-3 text-center">
                            <a href="#" OnClick="deleteEbook(id=${data})">  
    ...
    if (cultureInfo == "en-US") {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('/Ebook/delete', { id: EbookId }, function (response) {
                    if (response.success) {
                        Swal.fire(
                            'Deleted!',
...
function saveEbook(bool) {
    if ($('#Price').val() > 0 && $('#ISBN').val().length > 0 && $('#NbPages').val().length > 0 && $('#Editor').val().length > 0 && $('#Collection').val().length > 0 && $('#Theme').val().length > 0 && $('#Catalogue').val().length > 0 && $('#Doi').val().length > 0 && $('#OriginalTitle').val().length > 0 && newFunction(bool) && newFunction2(bool) && $('#OriginalLanguage').val().length > 0 && $('#State').val().length > 0 && $('#PublicationDate').val().length > 0) {
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    if (bool == "False") {
                        ...
                        setTimeout(function () { window.location = "/Ebook"; }, 2000);
                    } else {
                        Toast.fire({
                            icon: 'success',
                            title: 'Ebook Updated successfully',
                        })
                        setTimeout(function () { window.location = "/Ebook"; }, 2000);
                    }
                } else {
        ...
        if ($('#ISBN').val().length < 1) {
            $('#validISBN').text(InvalidInput)
        } else {
            $('#validISBN').text('')
        }
        if ($('#Price').val() < 1) {
            $('#validPrice').text(InvalidInput)
        } else {
            $('#validPrice').text('')
        }
        if ($('#NbPages').val().length < 1) {
            $('#validNbPages').text(InvalidInput)
        } else {
            $('#validNbPages').text('')
        }
